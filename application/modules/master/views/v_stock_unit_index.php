...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="box box-info">
            ...
            <div class="box-body">
                <table id="UnitTable" class="table table-bordered table-striped" style="width:100%">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <?php foreach ($stockunit->result() as $row) : ?>
                            <tr>
                                <td><?php echo $row->unit_code; ?></td>
                                <td><?php echo $row->unit_name; ?></td>
                                <td>
                                    <button class="btn btn-sm btn-success bold" onclick="view_data(<?php echo $row->unit_id; ?>)"><i class="fa fa-file-text-o"></i></button>
                                    &nbsp;
                                    <button class="btn btn-sm btn-warning bold" onclick="edit_data(<?php echo $row->unit_id; ?>)"><i class="fa fa-pencil"></i></button>
                                    &nbsp;
                                    <button class="btn btn-sm btn-danger bold" onclick="delete_data(<?php echo $row->unit_id; ?>)"><i class="fa fa-ban"></i></button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
            ...
            <div class="modal-content">
                ...
                <form action="#" id="form" class="form-horizontal">
                    <div class="modal-body">
                        ...
                        <input type="hidden" class="form-control" id="unitid" name="unitid" value="0">

                        <div class="form-group">
                            <label class="col-sm-4 control-label">Kode Satuan Barang</label>
                            <div class="col-sm-6">
                                <input type="text" class="form-control" id="unitcode" name="unitcode" readonly>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-4 control-label">Nama Satuan Barang</label>
                            <div class="col-sm-6">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#formimport').submit(function(e) {
            ...
            if (extension == "xls") {
                $.ajax({
                    url: '<?php echo site_url() ?>master/stock_unit/file_upload',
                    type: "post",
                    ...
                    success: function(data) {
                        window.location.href = '<?php echo site_url() ?>master/stock_unit/import/' + filename;
                    }
                });
            } else {
    ...
    function get_export() {
        var win = window.open('<?php echo site_url() ?>master/stock_unit/get_export', '_blank');
        if (win) {
            win.focus();
        } else {
    ...
    $(function() {
        $('#btnConfirm').click(function(e) {
            ...
            $.ajax({
                type: 'POST',
                dataType: 'JSON',
                url: '<?php echo site_url() ?>master/stock_unit/delete',
                data: {
                    stockunitid: dataId
                },
                success: function(data) {
    ...
    function add_data() {
        ...
        $('#unitcode').attr('disabled', false); //set button disable 
        $('#unitname').attr('disabled', false); //set button disable 

        $('#FormModal').modal('show'); // show bootstrap modal
        $('.modal-title').text('Tambah Satuan Barang'); // Set Title to Bootstrap modal title
    }

    function view_data(unitid) {
        $('#formtype').val("view");
        $('#form')[0].reset(); // reset form on modals
        ...
        $.ajax({
            type: "POST",
            dataType: "JSON",
            url: "<?php echo site_url() ?>master/stock_unit/ajax_view",
            data: {
                id: unitid
            },
            success: function(data) {
                $('#unitid').val(unitid);
                $('#unitcode').val(data.unit_code);
                $('#unitname').val(data.unit_name);

                $('#unitcode').attr('disabled', true); //set button disable 
                $('#unitname').attr('disabled', true); //set button disable 

                $('#FormModal').modal('show'); // show bootstrap modal
                $('.modal-title').text('Lihat Satuan Barang'); // Set Title to Bootstrap modal title
            },
            error: function() {
                Swal.fire({
                    icon: 'warning',
    ...
    function edit_data(unitid) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#FormModal').modal('show'); // show bootstrap modal
                $('.modal-title').text('Ubah Satuan Barang'); // Set Title to Bootstrap modal title
            },
            error: function() {
                Swal.fire({
                    icon: 'warning',
    ...
    function save_data() {
        ...
        if (formtype == 'add') {
            url = "<?php echo site_url() ?>master/stock_unit/ajax_add";
        } else {
            url = "<?php echo site_url() ?>master/stock_unit/ajax_update";
        }
        ...
        Swal.fire({
            ...
            if (e.isConfirm) {
                $.ajax({
                    type: "POST",
                    dataType: "JSON",
                    url: "<?php echo site_url() ?>master/stock_unit/ajax_delete",
                    data: {
                        id: unitid
                    },
                    success: function(data) {
